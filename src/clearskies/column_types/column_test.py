...
class ColumnTest(unittest.TestCase):
    ...
    def test_input_errors_requirements(self):
        ...
        self.assertEqual({"name": "'name' must be at least 10 characters long."}, errors)
        errors = self.column.input_errors("model", {"name": "me"})
        self.assertEqual({"name": "You are not allowed"}, errors)
    ...
    def test_documentation(self):
        ...
        self.assertEqual(AutoDocString, doc.__class__)
        self.assertEqual("my_name", doc.name)

        more_doc = self.column.documentation(name="hey", example="sup", value="okay")
        self.assertEqual(AutoDocString, more_doc.__class__)
        self.assertEqual("hey", more_doc.name)
        self.assertEqual("sup", more_doc.example)
        self.assertEqual("okay", more_doc.value)

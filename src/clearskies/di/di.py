...
class DI:
    ...
    def build(self, thing, context=None):
        if inspect.isclass(thing):
            return self.build_class(thing, context=context)
            ...
            instance = self.build_class(thing.object_class, context=context)
            ...
            return self.build_from_name(thing, context=context)
        elif callable(thing):
    ...
    def build_from_name(self, name, context=None):
        """
        Builds a dependency based on its name

        Order of priority:
          1. Already prepared things
          2. Things set via `bind(name, value)`
          3. Method on DI class called `provide_[name]`
        ...
        if name in self._bindings:
            self._prepared[name] = self.build(self._bindings[name], context=context)
            return self._prepared[name]

        if hasattr(self, f'provide_{name}'):
            self._prepared[name] = self.call_function(getattr(self, f'provide_{name}'))
            return self._prepared[name]

        if name in self._classes:
            self._prepared[name] = self.build_class(self._classes[name]['class'], context=context)
            return self._prepared[name]
    ...
    def build_class(self, class_to_build, context=None, name=None):
        """
        Builds a class
        ...
        if not build_arguments:
            self._prepared[name] = class_to_build()
            return self._prepared[name]

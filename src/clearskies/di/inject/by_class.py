...
class ByClass(Injectable):
    def __init__(self, cls: type, cache: bool=True):
        if not isinstance(cls, type):
            raise TypeError(f"I expected a class for the first argument to clearskies.di.inject.ByClass, but I received an object of type '{cls.__class__.__name__}' instead.")
        self.cls = cls
        self.cache = cache

...
class Get(Endpoint):
    ...
    def __init__(
        ...
        if self.record_lookup_column_name not in route_parameters:
            raise KeyError(f"Configuration error for Get endpoint: record_lookup_column_name is set to '{self.record_lookup_column_name}' but no matching routing parameter is found")
